...
describe('Return Order Service - DB test', () => {
    ...
    const product1 = new Product_Return(1, 10, "product 1", 9.99, "12345678901234567890123456789011"); // ok
    const product2 = new Product_Return(2, 10, "product 2", 13.99, "12345678901234567890123456789012"); // ok
    const product3 = new Product_Return(2, 10, "product 3", 16.99, "12345678901234567890123456789013"); // ok
    const product4 = new Product_Return(3, 10, "product 4", 110.99, "12345678901234567890123456789014"); // ok
    const product5 = new Product_Return(4, 10, "product 5", 12.99, "12345678901234567890123456789013"); // DUPLICATED RFID (same as product 3) --> used to test unsuccessfull addsSkuItemToskuReturnTable function
    const product6 = new Product_Return(4, 10, "product 6", 6.99, "12345678901234567890123456789014"); // DUPLICATED RFID (same as product 4)  --> used to test unsuccessfull addSkuItemToskuReturnTable function
...
function testAddSkuItemToskuReturnTable(product, returnOrderID, call) {
    test('Add products to SkuItemsToReturn table', async () => {
        if (call <= 4) {
            ...
            // CALLS WITH returnOrder_service ERROR CAUSE BY FAILED CONTSTRAINTS
            try {
