...
router.get('/api/items/:id/:supplierId', async (req, res) => {
    try {
        ...
        if (req.params.supplierId === 'null') {
            return res.status(422).json({ error: `Param supplier Id not Defined` }).end();
        }

        if (!validation.validate_defined_value(req.params.id)) {
            return res.status(422).json({ error: `Param item Id not Defined` }).end();
        }

        if (!validation.validate_defined_value(req.params.supplierId)) {
            return res.status(422).json({ error: `Param supplier Id not Defined` }).end();
        }

        let item = await item_service.getItemByIdAndSupplierId(req.params.id, req.params.supplierId);
        if (item === undefined) {
...
router.put('/api/item/:id/:supplierId', async (req, res) => {
    ...
    try {
        ...
        if (!(validation.validate_defined_value(req.params.supplierId) && validation.validate_integer_value(req.params.supplierId))) {
            return res.status(422).json({ error: `Wrong supplierId id parameter` }).end();
        }
        ...
        await item_service.updateItemByIdAndSupplierId(item, req.params.id, req.params.supplierId);
...
router.delete('/api/items/:id/:supplierId', async (req, res) => {
    ...
    try {
        ...
        if (!validation.validate_integer_value(req.params.supplierId)) {
            return res.status(422).json({ error: `Wrong supplier id parameter (not an integer)` }).end();
        }
        if (!validation.validate_defined_value(req.params.supplierId)) {
            return res.status(422).json({ error: `Wrong supplier id parameter` }).end();
        }
        await item_service.deleteItemByIdAndSupplierId(req.params.id, req.params.supplierId);
