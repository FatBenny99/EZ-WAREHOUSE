...
router.post('/api/restockOrder', async (req, res) => {
    ...
    try {
        ...
        for (let p of req.body.products) {
            let tmp = await item_service.getItemByIdAndSupplierId(p.itemId, req.body.supplierId);
            if (tmp === undefined) {
                return res.status(422).json({ error: "Supplier doesn't sell this kind of products" });
            }
        }
        ...
        const products = req.body.products.map((x) => new Product(x.SKUId, x.itemId, x.description, x.price, x.qty));
...
router.put('/api/restockOrder/:id/skuItems', async (req, res) => {
    try {
        ...
        for (let item of req.body.skuItems)
            await restock_order_service.addSkuItemsToSkuItemToRestock(new Sku_item_RestockOrder(item.SKUId, item.itemId, item.rfid), req.params.id);
