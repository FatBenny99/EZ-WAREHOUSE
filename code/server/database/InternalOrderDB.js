...
const Product = require('../models/Product_Internal');
        ...
        db.get(sql, [SkuID, rfid, internalorderID], (err, row) => {
            if (err) reject(err);
                ...
                if (row === undefined)
                    ...
                    resolve(new Product_SkuItemInternal(row.SKUId, row.description, row.price, row.RFID));
            }
        });
    });
};
        ...
        db.all(sql, [id], (err, rows) => {
            if (err) reject(err);
                ...
                const products = rows.map(row => {
                    let x = new Product_Internal(row.SKUId, row.description, row.price, row.qty);
                    return x;
                });
                resolve(products);
            };
        ...
        db.all(sql, [id], (err, rows) => {
            if (err) reject(err);
                ...
                let skuitems = rows.map(row => {
                    let x = new Product_SkuItemInternal(row.SKUId, row.description, row.price, row.RFID);
                    return x;
                });
                resolve(skuitems);
