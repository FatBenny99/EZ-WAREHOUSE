...
exports.getSkuItemstoOrder = (id) => {
    return new Promise((resolve, reject) => {
        const sql = 'SELECT SKUId,ItemID,RFID FROM  SkuItemToRestock  WHERE restockID=? ';
        db.all(sql, [id], (err, rows) => {
            if (err) reject(err);
            else {

                let skuitems = rows.map(row => {
                    let x = new Sku_Item_RestockOrder(row.SKUId, row.ItemID, row.RFID);
                    return x;
                });
                resolve(skuitems);
...
exports.getSkuItemstoReturn = (id) => {
    return new Promise((resolve, reject) => {
        const sql = 'SELECT ITEM.SKUId,SKU.itemId,RFID FROM  SKUToRestock  SKU,SKUITEM ITEM WHERE restockID=? AND SKU.SKUId=ITEM.SKUId AND ITEM.SKUId NOT IN (SELECT TestDescriptor.idSKU FROM TestDescriptor,TestResult WHERE TestDescriptor.id=TestResult.idTestDescriptor AND TestResult.Result=FALSE)';
        db.all(sql, [id], (err, rows) => {
            if (err) reject(err);
            else {
                let skuitems = rows.map(row => {
...
exports.addProductToSKURestockTable = (prod, restockID) => {
    ...
    return new Promise((resolve, reject) => {
        const sql = 'INSERT INTO SKUToRestock(SKUId,itemId,restockID,description,price,qty) VALUES(?,?,?,?,?,?)';
        db.run(sql, [prod.SKUId, prod.itemId, restockID, prod.description, prod.price, prod.qty], (err, rows) => {
            if (err) reject(err);
            else {
                resolve(true);
            };
...
exports.addSkuItemsToSkuItemToRestock = (skuitem, restockID) => {
    return new Promise((resolve, reject) => {
        const sql = 'INSERT INTO SkuItemToRestock(SKUId,ItemID,RFID,restockID) VALUES(?,?,?,?)';
        db.run(sql, [skuitem.SKUId, skuitem.itemId, skuitem.RFID, restockID], (err, rows) => {
            if (err) reject(err);
            else {
                resolve(true);
            };
...
exports.addProductstoOrder = (id) => {
    return new Promise((resolve, reject) => {
        const sql = 'SELECT SKUId,itemId,description,price,qty FROM  SKUToRestock WHERE restockID=?';
        db.all(sql, [id], (err, rows) => {
            if (err) reject(err);
            else {
                const products = rows.map(row => {
                    let x = new Product(row.SKUId, row.itemId, row.description, row.price, row.qty);
                    return x;
                });
                resolve(products);
            };
