...
exports.getProductstoReturnOrder = (id) => {
    return new Promise((resolve, reject) => {
        const sql = 'SELECT RFID,ItemId,SKUId,description,price FROM  SkuItemsToReturn WHERE returnOrderID=?';
        db.all(sql, [id], (err, rows) => {
            if (err) reject(err);
            else {
                const products = rows.map(row => {
                    let x = new Product_Return(row.SKUId, row.ItemId, row.description, row.price, row.RFID);
                    return x;
...
exports.addSkuItemToSkuReturnTable = (prod, id) => {
    return new Promise((resolve, reject) => {
        const sql = 'INSERT INTO SkuItemsToReturn(RFID,SKUId, ItemId,description,price,returnOrderID) VALUES(?,?,?,?,?,?)';
        db.run(sql, [prod.RFID, prod.SKUId, prod.itemId, prod.description, prod.price, id], (err, rows) => {
            if (err) reject(err);
            else {
                resolve(true);
            };
